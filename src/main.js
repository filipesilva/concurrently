...
function mergeDefaultsWithArgs(config, options) {
    // This will pollute config object with other attributes from options too
...
function run(commands, options) {
    options = options || {};
    // Compose config.
    config = mergeDefaultsWithArgs(config, options);
    ...
    return new Promise(function (resolve, reject) {
        ...
        // Exit according to config.
        function exitFn(childExitCodes) {
            var success;
            ...
            success === 0 ? resolve(success) : reject(success);
        }


        handleClose(streams, children, childrenInfo, exitFn);
...
function handleClose(streams, children, childrenInfo, exitFn) {
